...
encode(#dns_message{id = MsgId, additional = Additional} = Msg, Opts) ->
    ...
    case maps:get(tsig, Opts, undefined) of
        ...
        #{alg := Alg, name := Name} = TSIGOpts ->
            ...
            TSIGSize = encode_message_tsig_size(EncodedName, LowerAlg, Other),
            ...
            {MsgBin0, NewMAC} = encode_message_tsig_add(
...
encode_message_tsig_add(
    ...
    case dns:gen_tsig_mac(LowerAlg, MsgBin, EncodedName, Secret, Time, Other, Options) of
        {ok, MAC} ->
            MS = byte_size(MAC),
            OLen = byte_size(Other),
            AlgBin = encode_dname(LowerAlg),
            TSIGData =

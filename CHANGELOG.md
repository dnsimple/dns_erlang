...
- Optimized `dns_encode` message encoding: the bounded encoders thread one accumulator through all sections and every record's RDATA is appended with its RDLENGTH in a single append, eliminating intermediate binaries and per-section copies. ~1.1–1.3x faster on typical bounded responses with 5–35% less allocation across message shapes.
